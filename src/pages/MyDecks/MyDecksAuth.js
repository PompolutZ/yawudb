...
const DeckConflictsAndWarnings = lazy(() =>
    import("./atoms/DeckConflictsAndWarnings")
);
...
class MyDecksAuth extends Component {
    ...
    async componentDidMount() {
        const userDataRef = await this.props.firebase.db
            .collection("users")
        ...
        for (let id of userData.mydecks) {
            ...
            if (data.author === "Anonymous") {
                ...
                const updatedAnonIds = anonDeckIds.filter(
                    (anonDeckId) => anonDeckId !== id
                );
                localStorage.setItem(
                    "yawudb_anon_deck_ids",
                    JSON.stringify(updatedAnonIds)
                );
                localStorage.setItem(
                    "yawudb_decks",
                    JSON.stringify({ ...cache, [id]: updatedData })
                );

                this.props.addOrUpdate(id, created, {
                    ...updatedData,
                    id: id,
                    created: created,
                });
                ...
                {!this.state.loading && decks.length > 0 && (
                    <div>
                        <div className={classes.header}>
                            <Switch
                                isChecked={this.state.showConflicts}
                                onChange={this.handleChangeShowConflicts}
                                label="Show conflicts and warnings"
                            />
                            <Typography
                                variant="subtitle2"
                            ...
                            >
                                <i>
                                    To use this feature mark sets you own in the{" "}
                                    <span
                                        style={{
                                            color: "#3B9979",
                                            cursor: "pointer",
                                        }}
                                        onClick={this.handleProfileLinkClicked}
                                    >
                                        <u>Profile</u>
                                    </span>{" "}
                                    page.
                            ...
                            {this.state.showConflicts && (
                                <div>
                                    <Typography className={classes.headerItem}>
                                        ...
                                        means that you have only single copy of
                                        a card in your collection.
                                    </Typography>
                                    <Typography className={classes.headerItem}>
                                        <span style={{ color: "orange" }}>
                                            Warning
                                        </span>{" "}
                                        means that you have several copies of a
                                        card, but used them in more decks than
                                        copies.
                                    </Typography>
                                </div>
                            )}
    ...
    checkForConflictsOrWarnings = () => {
        ...
        const conflictCandidates = toPairs(cardsInDecks).filter(
            ([k, v]) => v.length > 1
        );

        // convert ownSetsToCards
        const ownCards = toPairs(this.props.ownSets).reduce(
            (acc, [set, copies]) => {
                const s = setInfos[set];
                const { wave } = s;
                const setCards = Object.keys(s)
                    .filter((k) => k !== "wave")
                    .reduce((acc, k) => [...acc, ...s[k]], [])
                    .map((card) => getDbIndexByWaveAndCard(wave, card));
        ...
        const conflictsOrWarnings = conflictCandidates.map(([k, v]) => ({
            id: k,
            copies: ownCards[k],
            decks: v,
        }));
        const conflicts = conflictsOrWarnings.filter(
            ({ copies, decks }) => copies === 1 && decks.length > 1
        );
        const deckConflicts = this.findConflictsOrWarnings(conflicts);
        const warnings = conflictsOrWarnings.filter(
            ({ copies, decks }) => copies > 1 && decks.length > copies
        );
...
export default connect(
    mapStateToProps,
    mapDispatchToProps
)(withFirebase(withRouter(withStyles(styles)(MyDecksAuth))));
