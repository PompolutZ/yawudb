...
class Decks extends Component {
    ...
    getDecksIds = async () => {
        try {
            const idsSnap = await this.props.firebase.realdb.ref(`/decks_meta/${this.props.match.params.faction}/ids`).once('value');
            return idsSnap.val();
        } catch(error) {
            console.log(error);
        }
    ...
    loadPage = async (pageNumber, lastItemOnPreviousPage) => {
        try {
            ...
            if(lastItemOnPreviousPage) {
                console.log('HERE');
                snapshot = await this.props.firebase.db.collection('decks').orderBy('created', 'desc').startAfter(lastItemOnPreviousPage).limit(10).get();
            } else {
                snapshot = await this.props.firebase.db.collection('decks').orderBy('created', 'desc').limit(10).get();
            }
...
export default connect(mapStateToProps, mapDispatchToProps)(withFirebase(withRouter(withStyles(styles)(Decks))));