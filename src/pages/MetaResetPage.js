...
export default function MetaReset(props) {
    ...
    useEffect(() => {
        firebase.realdb
            ...
            .once("value")
            .then((s) => {
                const publicDecks = Object.entries(s.val()).map(
                    ([
                        id,
                        {
                            author,
    ...
    useEffect(() => {
        console.log("ALL DECKS", allDecks ? allDecks.length : 0);
        console.log(
            allDecks.filter(
                (deck) =>
                    (new Date().getTime() - deck.modified) /
                        1000 /
                        60 /
                        60 /
                        24 <
    ...
    const handleUpdatePublicDecks = () => {
        const cleaned = allDecks.reduce(
            (dup, deck, i, source) => {
                if (
                    dup.original.find(
                        (d) =>
                            d.faction === deck.faction && d.cards === deck.cards
                    )
                ) {
                    return {
                        ...dup,
        ...
        const back150 = cleaned.original.filter(
            (deck) =>
                (new Date().getTime() - deck.modified) / 1000 / 60 / 60 / 24 <
                150
        );
        const olderThan150 = cleaned.original.filter(
            (deck) =>
                (new Date().getTime() - deck.modified) / 1000 / 60 / 60 / 24 >=
                150
        );
        const payload = back150.reduce(
            (p, deck) => ({ ...p, [deck.id]: deck }),
            {}
        );
        console.log(payload);
        const sharedDecks = cleaned.original.reduce(
            (log, deck) => ({
                ...log,
    ...
    return (
        <div>
            ...
            <button
                className=" m-1 p-1 bg-accent rounded text-white"
                onClick={handleUpdatePublicDecks}
            >
                Update Public Decks
            </button>
            {allDecks
                .sort((l, r) => r.modified - l.modified)
                .map((deck) => (
                    <section
                        key={deck.id}
                        className="bg-blue-100 m-1 p-1 text-grey-800"
                    >
                        <div className="text-grey-500 font-semibold text-sm">
                            {deck.id}
                        </div>
                        <div className="text-grey-700 text-xl">{deck.name}</div>
                        <div className="text-blue-500">
                            {new Date(deck.modified).toLocaleDateString()}
                        </div>
                    </section>
                ))}
        </div>
