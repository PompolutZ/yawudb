...
class FactionsFilterToggle extends Component {
    ...
    handleToggle = (faction) => {
        ...
        if (indexOf >= 0) {
            factions = [
                ...this.state.selectedFactions.slice(0, indexOf),
                ...this.state.selectedFactions.slice(indexOf + 1),
            ];
        } else {
        ...
        this.timeoutId = setTimeout(
            () => this.props.onFactionsChange(factions),
            350
        );
    };
    ...
    renderRow = (faction) => {
        return (
            ...
            >
                ...
                />
                <Typography variant="button" style={{ marginLeft: ".5rem" }}>
                    {factions[faction]}
                </Typography>
            </div>
        );
