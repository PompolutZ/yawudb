...
const RestrictedBannedCardsCount = ({ banned, restricted, rotatedOut, style }) => (
    ...
    }
    {
        rotatedOut > 0 && (
            <div style={{ display: 'flex', alignItems: 'center'}}>
                <img src={`/assets/icons/rotatedOut.png`} style={{ width: '.7rem', height: '.7rem' }} />
                <Typography style={{ color: '#3B9979', fontSize: '.7rem'}}>{rotatedOut}</Typography>
            </div>
        )
    }
    </div>
