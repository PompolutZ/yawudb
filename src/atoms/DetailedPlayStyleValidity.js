...
function DetailedPlayStyleValidity({ validFormats, cards }) {
    ...
    const cFC = onlyUniversals.filter((c) =>
        checkCardForsakenFor(c.id, "championship")
    ).length;
    const outOfSeason = onlyUniversals.filter(
        (c) => Number(c.id) < 3000 && !ignoreAsDublicate(c.name)
    ).length;
    const surges = allCards.filter((c) => c.type === 0 && c.scoreType === 0)
        .length;
                ...
                <Grid container alignItems="center">
                    <Grid item xs={2}>
                        <RelicIcon
                            className={classes.icon}
                            style={{
                                color: validFormats.includes("relic")
                                    ? "#3B9979"
                                    : "red",
                            }}
                        />
                ...
                <Grid item container alignItems="center">
                    <Grid item xs={2}>
                        <OpenFormatIcon
                            className={classes.icon}
                            style={{
                                color: validFormats.includes("open")
                                    ? "#3B9979"
                                    : "red",
                            }}
                        />
